...
export default function LoginPage() {
  ...
  // Signed in user routing: if profile complete → /chats, else → /onboarding
  useEffect(() => {
    let active = true;
    (async () => {
      const {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col lg:flex-row bg-canvas">
      {/* LEFT SIDE: Member Sign In Form & Logo */}
      <div className="w-full lg:w-1/2 xl:w-5/12 flex flex-col justify-between p-6 sm:p-10 lg:p-12 xl:p-16 border-r border-outline/10">
        ...
        <div className="flex items-center justify-between gap-md mb-8 lg:mb-12">
          ...
          <span className="text-xs text-ink-muted bg-surface-container-high px-3 py-1 rounded-full border border-ink/10">
            เข้าสู่ระบบ
          </span>
        </div>
        ...
        <div className="w-full max-w-md mx-auto space-y-8 my-auto py-6">
          <div className="space-y-2 text-left">
            <h1 className="text-2xl sm:text-3xl font-bold text-ink">ยินดีต้อนรับกลับมา</h1>
            <p className="text-sm sm:text-base text-ink-muted">
              เข้าสู่ระบบบัญชี Arm Chat ของคุณเพื่อใช้งานห้องแชทและสายโทร
            </p>
          </div>
          ...
          <div className="bg-surface-white rounded-tile border border-ink/20 p-6 sm:p-8 shadow-sm space-y-6">
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-sm">
                  <Button
                    ...
                    aria-label="ย้อนกลับ"
                  />
                  <h2 className="text-base font-semibold text-ink">เบอร์โทรศัพท์ของคุณ</h2>
                </div>

                <form onSubmit={handleStartPhone} className="space-y-5">
                  <div className="space-y-1.5">
                    <label htmlFor="login-phone" className="block text-xs font-medium text-ink-muted px-1">
                      หมายเลขโทรศัพท์
                    </label>
                  ...
                  <p className="text-xs text-ink-muted text-center italic">
                    &ldquo;ระบบป้อนรหัสทดสอบ OTP ให้อัตโนมัติในขั้นตอนถัดไป&rdquo;
                  </p>
            ...
            {step === 'verify' && (
              <div className="space-y-6">
                <div className="text-center space-y-3">
                  <h2 className="text-base font-semibold text-ink">ยืนยันตัวตนด้วยรหัส OTP</h2>
                  <p className="text-xs text-ink-muted">
                    ระบบได้ป้อนรหัสยืนยัน 6 หลักให้อัตโนมัติเรียบร้อยแล้ว
                  </p>
            ...
            {step === 'success' && (
              <div className="space-y-4 text-center py-6">
                ...
                <h2 className="text-2xl font-bold text-ink">เข้าสู่ระบบสำเร็จ!</h2>
                <p className="text-sm text-ink-muted">กำลังนำคุณเข้าสู่ Arm Chat...</p>
              </div>
            )}
          </div>
          ...
          <div className="text-center text-xs text-ink-muted">
            ยังไม่มีบัญชี Arm Chat ใช่ไหม?{' '}
            <Link href="/register" className="font-semibold text-primary underline hover:text-ink transition-colors">
