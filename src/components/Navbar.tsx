      ...
      if (session?.user) {
        ...
        if (typeof window !== 'undefined') {
          ...
          window.OneSignalDeferred.push(async function (OneSignal: any) {
            // ตัวผู้ใช้เลือกเองจากหน้า Settings เท่านั้น — ห้ามเด้ง popup อัตโนมัติ
        ...
        setAvatarUrl('');
        setUnreadCount(0);
        // P0.4: logout OneSignal so push token no longer maps to this user
        if (typeof window !== 'undefined') {
          window.OneSignalDeferred = window.OneSignalDeferred || [];
          window.OneSignalDeferred.push(async function (OneSignal: any) {
            try { await OneSignal.logout(); } catch { /* noop if not logged in */ }
          });
        }
      }
    });
